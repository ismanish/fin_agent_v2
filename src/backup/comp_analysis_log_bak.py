...
def _log_source_value(ticker: str, metric: str, source_name: str, value: Optional[float], 
    ...
    _comp_log["metrics"][ticker][metric]["sources"][source_name] = source_entry

def _log_component_value(ticker: str, metric: str, component_name: str, value: Optional[float]) -> None:
    """Log a component value used in calculation"""
    global _comp_log
    ...
    _comp_log["metrics"][ticker][metric]["components"][component_name] = value
    ...
    for k in keys:
        ...
        if v is not None:
            ...
            if ticker and metric:
                ...
                _log_source_value(ticker, metric, f"FY_{year}_{k}", v, "10-K", 
...
def _compute_ltm(ytd_2025: Optional[float], fy_2024: Optional[float], ytd_2024: Optional[float], ticker: str = "", metric: str = "") -> Optional[float]:
    try:
        ...
        # Log the components and calculation
        if ticker and metric:
            _log_component_value(ticker, metric, "fy_2024", fy_2024)
            _log_component_value(ticker, metric, "ytd_2025", ytd_2025)
            _log_component_value(ticker, metric, "ytd_2024", ytd_2024)
            _log_final_metric(ticker, metric, result, "FY2024 + YTD2025 - YTD2024")
...
def _compute_company_metrics(ticker: str) -> Dict[str, Optional[float]]:
    ...
    # LTM constructs
    rev_ltm = _compute_ltm(rev_ytd_2025, rev_fy_2024, rev_ytd_2024, t, "LTM Revenue")
    ebitda_ltm = _compute_ltm(ebitda_ytd_2025, ebitda_fy_2024, ebitda_ytd_2024, t, "LTM EBITDA")
    rent_ltm = _compute_ltm(rent_ytd_2025, rent_fy_2024, rent_ytd_2024, t, "EBITDAR / (Int + Rents)")
    int_ltm = _compute_ltm(int_ytd_2025, int_fy_2024, int_ytd_2024, t, "EBITDAR / (Int + Rents)")
    ...
    fcf_ltm = _compute_ltm(fcf_ytd_2025, fcf_fy_2024, fcf_ytd_2024, t, "(FCF + Rents) / (Total Debt + COL)")
    ...
    if rev_ltm not in (None, 0) and ebitda_ltm is not None:
        try:
            ...
            _log_component_value(t, "EBITDA Margin %", "ltm_revenue", rev_ltm)
            _log_component_value(t, "EBITDA Margin %", "ltm_ebitda", ebitda_ltm)
            _log_final_metric(t, "EBITDA Margin %", ebitda_margin, "(LTM EBITDA / LTM Revenue) * 100")
        except Exception:
            ebitda_margin = None
    ...
    if denom not in (None, 0) and ebitdar_ltm is not None:
        ...
        _log_component_value(t, "EBITDAR / (Int + Rents)", "ltm_ebitdar", ebitdar_ltm)
        _log_component_value(t, "EBITDAR / (Int + Rents)", "ltm_interest", int_ltm)
        _log_component_value(t, "EBITDAR / (Int + Rents)", "ltm_rent", rent_ltm)
        _log_final_metric(t, "EBITDAR / (Int + Rents)", ebitdar_over_int_plus_rents, "EBITDAR / (Interest + Rents)")
    
    td_col_over_ebitdar = None
    if ebitdar_ltm not in (None, 0):
        td_col_over_ebitdar = (total_debt + col) / ebitdar_ltm
        _log_component_value(t, "(Total Debt + COL) / EBITDAR", "total_debt", total_debt)
        _log_component_value(t, "(Total Debt + COL) / EBITDAR", "col", col)
        _log_component_value(t, "(Total Debt + COL) / EBITDAR", "ltm_ebitdar", ebitdar_ltm)
        _log_final_metric(t, "(Total Debt + COL) / EBITDAR", td_col_over_ebitdar, "(Total Debt + COL) / EBITDAR")
    
    nd_col_over_ebitdar = None
    if ebitdar_ltm not in (None, 0):
        nd_col_over_ebitdar = (net_debt + col) / ebitdar_ltm
        _log_component_value(t, "(Net Debt + COL) / EBITDAR", "net_debt", net_debt)
        _log_component_value(t, "(Net Debt + COL) / EBITDAR", "col", col)
        _log_component_value(t, "(Net Debt + COL) / EBITDAR", "ltm_ebitdar", ebitdar_ltm)
        _log_final_metric(t, "(Net Debt + COL) / EBITDAR", nd_col_over_ebitdar, "(Net Debt + COL) / EBITDAR")
    
    td_col_over_total_cap = None
    if total_cap not in (None, 0):
        td_col_over_total_cap = (total_debt + col) / total_cap
        _log_component_value(t, "(Total Debt + COL) / Total Cap", "total_debt", total_debt)
        _log_component_value(t, "(Total Debt + COL) / Total Cap", "col", col)
        _log_component_value(t, "(Total Debt + COL) / Total Cap", "total_cap", total_cap)
        _log_final_metric(t, "(Total Debt + COL) / Total Cap", td_col_over_total_cap, "(Total Debt + COL) / Total Cap")
    ...
    if den not in (None, 0) and num is not None:
        ...
        _log_component_value(t, "(FCF + Rents) / (Total Debt + COL)", "ltm_fcf", fcf_ltm)
        _log_component_value(t, "(FCF + Rents) / (Total Debt + COL)", "ltm_rent", rent_ltm)
        _log_component_value(t, "(FCF + Rents) / (Total Debt + COL)", "total_debt", total_debt)
        _log_component_value(t, "(FCF + Rents) / (Total Debt + COL)", "col", col)
        _log_final_metric(t, "(FCF + Rents) / (Total Debt + COL)", fcf_plus_rents_over_td_col, "(FCF + Rents) / (Total Debt + COL)")
    ...
    for y in yrs:
        ...
        if r1 is not None:
            vals_r1.append(r1)
            _log_component_value(t, "3Y Avg (TD+COL)/EBITDAR", f"fy_{y}_ratio", r1)
        if r2 is not None:
            vals_r2.append(r2)
            _log_component_value(t, "3Y Avg (TD+COL)/Total Cap", f"fy_{y}_ratio", r2)
        if r3 is not None:
            vals_r3.append(r3)
            _log_component_value(t, "3Y Avg (FCF+Rents)/(TD+COL)", f"fy_{y}_ratio", r3)
...
def _resolve_token_value_ltm(token: str,
    ...
    def ltm_of(keys: List[str], section: str) -> Optional[float]:
        ...
        return _compute_ltm(y25, fy24, y24, ticker, metric)
    ...
    return _compute_ltm(y25, fy, y24, ticker, metric)
...
def _compute_company_metrics_from_mapping(ticker: str, mapping: Dict[str, Dict[str, Any]]) -> Dict[str, Optional[float]]:
    ...
    def eval_ltm_expr(expr: str, metric_name: str) -> Optional[float]:
        ...
        if result is not None:
            for tok, val in vals.items():
                _log_component_value(t, metric_name, tok.lower(), val)
    ...
    def eval_fy_expr(expr: str, year: int, metric_name: str) -> Optional[float]:
        ...
        if result is not None:
            for tok, val in vals.items():
                _log_component_value(t, metric_name, f"{tok.lower()}_{year}", val)
    ...
    if margin_expr:
        ...
        if ltm_revenue not in (None, 0) and ltm_ebitda is not None:
            try:
                ...
                _log_component_value(t, "EBITDA Margin %", "ltm_revenue", ltm_revenue)
                _log_component_value(t, "EBITDA Margin %", "ltm_ebitda", ltm_ebitda)
                _log_final_metric(t, "EBITDA Margin %", ebitda_margin, "(LTM EBITDA / LTM Revenue) * 100")
            except Exception:
                ebitda_margin = None
    ...
    def avg3(name: str) -> Optional[float]:
        ...
        for y in [2024, 2023, 2022]:
            ...
            if v is not None:
                vals.append(float(v))
                _log_component_value(t, name, f"fy_{y}_ratio", v)
        if not vals:
            return None
...
def run_comp_analysis(ticker: str, write_files: bool = True, upload_to_azure: bool = False) -> Dict[str, Any]:
    ...
    if upload_to_azure:
        try:
            ...
            json_url = upload_json_to_blob_direct(result.get("rows", []), container_name, json_blob_name)
            blob_urls["json_url"] = json_url
            
            # Upload CSV directly
            csv_blob_name = f"{ticker.upper()}/COMP_{ticker.upper()}_{timestamp_str}.csv"
            csv_url = upload_csv_to_blob_direct(result.get("rows", []), container_name, csv_blob_name)
