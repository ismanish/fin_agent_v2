...
def get_filings_for_ticker(ticker):
    ...
    # If in testing mode, only use local files
    if TESTING_MODE:
        print(f"TESTING MODE: Using only local files for {ticker}")
        k_file_path = k_files[0] if k_files else None
        q_file_path = q_files[0] if q_files else None
        
        if not k_file_path:
            print(f"No local 10-K file found for {ticker}. Please add a PDF file to {full_data_folder}")
            return None, None
        ...
        if q_file_path:
            ...
            print(f"No local 10-Q found for {ticker}")
...
def get_response_from_llm(pdf_text: str, ticker: str) -> Tuple[Optional[str], Optional[dict], Optional[str]]:
    ...
    try:
        ...
        cap_table_json, source_lineage, csv_part = parse_llm_response_with_lineage(content)
        
        if not cap_table_json:
            print("Warning: Could not extract cap table JSON from LLM response.")
            return None, None, None
        
        return cap_table_json, source_lineage, csv_part
...
def build_cap_table(ticker: str, write_files: bool = True, generate_lineage: bool = True, upload_to_azure: bool = False) -> Dict[str, Any]:
    ...
    # Check for cached data (existing logic)
    json_output_full_path = os.path.join(ROOT, json_output_path)
    csv_output_full_path = os.path.join(ROOT, csv_output_path)
    
    # # If using cached data and lineage is requested, check for existing lineage log
    # using_cached_data = False  # Your existing logic here
    
    # if using_cached_data and not generate_lineage:
    #     # Return cached data without lineage
    ...
    #             csv_rows = []
    #             for line in updated_csv_data.split('\n'):
    ...
    #             print(f"✅ CAP table cached data uploaded to Azure Blob Storage: {blob_urls}")
    #         except Exception as e:
    #             print(f"Warning: Failed to upload cached CAP table data to Azure Blob Storage: {e}")
    ...
    # Get updated data from LLM including source lineage
    cap_table_json, source_lineage, csv_data = get_response_from_llm(combined_text, ticker)
    
    if not cap_table_json:
        raise Exception(f"Failed to generate cap table for {ticker}")
    
    # Parse and compute cap table data
    cap_table_data = json.loads(cap_table_json)
